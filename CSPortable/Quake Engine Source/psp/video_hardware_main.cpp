...
void GL_DrawAliasShadow (aliashdr_t *paliashdr, int posenum)
{
	...
	while (1)
	{
		...
		do
		{
            ...
            sceGuDrawArray(prim, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_COLOR_8888, count, 0,0);
	}	
}
...
transparent = false;
	...
	if (i >= 1 && i<=cl.maxclients /* && !strcmp (currententity->model->name, "progs/player.mdl") */)
		if (ambientlight < 50)
			ambientlight = shadelight = 50;

// HACK HACK HACK -- no fullbright colors, so make torches full light
	if (!strcmp (clmodel->name, "progs/lamp.mdl")
            || !strcmp (clmodel->name, "maps/a.mdl")
            || !strcmp (clmodel->name, "progs/muzzle.mdl")
