...
enum {m_none, m_main, m_singleplayer,m_aim,m_server,m_stest,
...
m_search, m_slist, m_osk, m_team, m_counter, m_terror, m_bots, m_sedit,m_networks,m_servers} m_state;
...
void M_Main_Draw (void)
{
    ...
    M_DrawTransPic (mp.value, 90, Draw_CachePic ("gfx/mp.lmp") );
    M_DrawTransPic (op.value, 130, Draw_CachePic ("gfx/op.lmp") );
    M_DrawTransPic (cr.value, 170, Draw_CachePic ("gfx/cr.lmp") );
...
void M_Pistolst_Key (int key)
{
	...
	case K_ENTER:
		...
		Cbuf_AddText ("impulse 23\n"); 
				break;
		case 1:
		Cbuf_AddText ("impulse 40\n"); 
				break;
		case 2:
		Cbuf_AddText ("impulse 85\n"); 
				break;
		case 3:
		Cbuf_AddText ("impulse 86\n"); 
				break;
		case 4:
...
M_DrawTransPic (jg.value, 50,Draw_CachePic("gfx/joingame.lmp") );
M_DrawTransPic (cg.value, 90,Draw_CachePic("gfx/creategame.lmp") );
M_DrawTransPic (set.value, 130,Draw_CachePic("gfx/setup.lmp") );
M_DrawTransPic (ser.value, 170,Draw_CachePic("gfx/serverlist.lmp") );
M_DrawTransPic (dis.value, 210,Draw_CachePic("gfx/disconnect.lmp") );
...
else if(m_multiplayer_cursor == 1)
{
jg.value = -50;
cg.value = -30;
set.value = -50;
...
else if(m_multiplayer_cursor == 2)
{
jg.value = -50;
cg.value = -50;
set.value = -30;
...
else if(m_multiplayer_cursor == 3)
{
jg.value = -50;
cg.value = -50;
set.value = -50;
...
else if(m_multiplayer_cursor == 4)
{
jg.value = -50;
cg.value = -50;
set.value = -50;
    ...
    if (m_submenu == 0)
    {
    	switch (options_cursor)
        {
			case OPT_SCRSIZE:	// screen size
				haircolor.value += dir * 1;
				if (haircolor.value < 0)
					crosshair.value = 0;
				if (crosshair.value > 255)
					haircolor.value = 255;
				Cvar_SetValue ("haircolor", haircolor.value);
				break;
...
float d;
	...
	switch (m_submenu) {
        case 0:    
			M_Print (16, 32+(OPT_SCRSIZE*8),   "         Crosshair Color");
			r = haircolor.value;
			//M_DrawSlider (220, 32+(OPT_SCRSIZE*8), r);
			     Draw_Fill (350, 32+(OPT_SCRSIZE*8),32,8, r);
...
{"impulse 104", 		"Extras Menu"},
{"impulse 10", 		"Silencer/Zoom"},
{"impulse 120", 		"IngameMenu"},
...
void M_Help_Draw (void)
{
	  M_PrintWhite (50, 30, "CSPortable by Stealth Kill");

	  M_PrintWhite (50, 50, "       Thanks to");
	  M_Print (-20, 100, "UrbanKid");
	  M_Print (-20, 110, "McCunyao          ");
	  M_Print (-20, 120, "dark chazz");
	  M_Print (-20, 130, "El Matador");
	  M_Print (-20, 140, "jurajstyk");
	  M_Print (-20, 150, "ID Software");
	  M_Print (-20, 160, "SoTe");
	  M_Print (-20, 170, "Kratelos");
	  M_Print (-20, 180, "Delmi");
	  M_Print (-20, 190, "Shadowflare");
...
if(m_networks_cursor == 0)
{
inf.value = -30;
ah.value = -50;
}
else if(m_networks_cursor == 1)
{
inf.value = -50;
ah.value = -30;
}
	...
	f = (int)(host_time * 10)%6;
      ...
      M_DrawTransPic (inf.value, 60, Draw_CachePic ("gfx/infrastructur.lmp") );
	M_DrawCheckboxNew (130, 60, tcpipAvailable && tcpprotocol == 0);

      M_DrawTransPic (ah.value, 180, Draw_CachePic ("gfx/adhoc.lmp") );
	M_DrawCheckboxNew (130, 180, tcpipAvailable && tcpprotocol == 1);
...
void M_Networks_Key (int key)
{
	...
	case K_DOWNARROW:
		S_LocalSound ("misc/menu1.wav");
		if (++m_networks_cursor >= NETWORKS_ITEMS)
			m_networks_cursor = 0;
		break;
	...
	case K_UPARROW:
		S_LocalSound ("misc/menu1.wav");
		if (--m_networks_cursor < 0)
			m_networks_cursor = NETWORKS_ITEMS - 1;
		break;

	case K_ENTER:
		m_entersound = true;
		switch (m_networks_cursor)
		{
		case 0:
			Datagram_Shutdown();
...
void M_Init (void)
{
	...
	Cmd_AddCommand ("menu_buyct", M_Menu_Buyt_f);
	Cmd_AddCommand ("menu_buyt", M_Menu_Buyct_f);
}
